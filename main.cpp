    ...
    while (1)
    {
            ...
            pData->threadSyncFlag)
        {
            ...
            if (elementsRead > 0)
            {
                ...
                for (s = 0; s < 2 && ptr[s] != NULL; ++s)
                {
                    
                        SAMPLE sample = ((SAMPLE*)ptr[s])[1];
                        if (sample == 0)break;
                        float dB = 20.0f * log10f(fabs(sample) / (float)32768);
                        //float dB = fabs(sample);
                        //float dB = 20.0f * log10f(fabs(sample) / (float)38);
                        //fprintf(pData->file, "%.2f ", dB);
                        //printf("%.2f ", dB);
                        outfile << dB << "\n";
...
int main(){
    ...
    int deviceIndex = 1; // choose the second device (index starts at 0)
    const PaDeviceInfo* deviceInfo = Pa_GetDeviceInfo(deviceIndex);
    if( deviceInfo == NULL ){
        printf( "Error: Pa_GetDeviceInfo returned NULL\n" );
        return 1;
    }
    printf("Selected input device: %s\n", deviceInfo->name);
    printf("Supported sample formats:\n");
    for(int i=0; i<paFloat32+1; i++){
        if(deviceInfo->maxInputChannels > 0 && Pa_IsFormatSupported(NULL, &deviceInfo->defaultLowInputLatency, i, deviceIndex, NULL) == paFormatIsSupported){
            printf("  - %s\n", Pa_GetSampleFormatName(i));
        }
    }
    Pa_Terminate();
    return 0;
